jQuery( document ).ready(function() {
    ...
    let input = jQuery('input[name^="_dci_progetto_argomenti"]');
    input.each(function() {
        jQuery(this).click(function(){
            dci_remove_highlight_missing_field('.cmb2-id--dci-progetto-argomenti');
        });
    });
    ...
    jQuery( 'form[name="post"]' ).on('submit', function(e) {
         ...
         */
        if(document.activeElement.id === 'publish' && jQuery('input[name^="_dci_progetto_argomenti"]:checked').length == 0){
            dci_highlight_missing_field('.cmb2-id--dci-progetto-argomenti');
            return false;
        }
        return true;
    });
