...
function dci_register_post_type_icad() {
    ...
    // Mostra il menu solo agli utenti con permesso
...
function dci_icad_metaboxes() {
    ...
    $cmb_apertura->add_field(array('id' => $prefix . 'anno_conferimento','name' => __('Anno di Conferimento', 'design_comuni_italia'),'type' => 'text_date_timestamp','date_format' => 'Y'));
    $cmb_apertura->add_field(array('id' => $prefix . 'soggetto_dichiarante','name' => __('Soggetto dichiarante', 'design_comuni_italia'),'type' => 'text'));
    $cmb_apertura->add_field(array('id' => $prefix . 'soggetto_percettore','name' => __('Soggetto percettore', 'design_comuni_italia'),'type' => 'text'));
    $cmb_apertura->add_field(array('id' => $prefix . 'dirigente_non_dirigente','name' => __('Dirigente/Non Dirigente', 'design_comuni_italia'),'type' => 'select','options' => array('dirigente' => __('Dirigente','design_comuni_italia'),'non_dirigente' => __('Non Dirigente','design_comuni_italia'))));
    $cmb_apertura->add_field(array('id' => $prefix . 'soggetto_conferente','name' => __('Soggetto Conferente', 'design_comuni_italia'),'type' => 'text'));
    $cmb_apertura->add_field(array('id' => $prefix . 'data_conferimento_autorizzazione','name' => __('Data conferimento autorizzazione dell’incarico', 'design_comuni_italia'),'type' => 'text_date_timestamp','date_format' => 'd/m/Y'));
    $cmb_apertura->add_field(array('id' => $prefix . 'oggetto_incarico','name' => __('Oggetto dell’incarico', 'design_comuni_italia'),'type' => 'text'));
    $cmb_apertura->add_field(array('id' => $prefix . 'durata','name' => __('Durata', 'design_comuni_italia'),'type' => 'text'));
    $cmb_apertura->add_field(array('id' => $prefix . 'compenso_lordo','name' => __('Compenso Lordo', 'design_comuni_italia'),'type' => 'text'));
    ...
    $cmb_documenti->add_field(array('id' => $prefix . 'allegati','name' => __('Allegati', 'design_comuni_italia'),'type' => 'file_list'));
}
...
function dci_icad_admin_script() {
    if (get_current_screen()->post_type === 'incarichi_dip') {
        wp_enqueue_script('icad-admin-script', get_template_directory_uri() . '/inc/admin-js/incarichi_dip.js', array(), null, true);
    }
}
...
function dci_icad_set_post_content($data) {
    if ($data['post_type'] === 'incarichi_dip') {
        $prefix = '_dci_icad_';
        $descrizione_breve = isset($_POST[$prefix . 'oggetto_incarico']) ? sanitize_text_field($_POST[$prefix . 'oggetto_incarico']) : '';
        $soggetto_dichiarante = isset($_POST[$prefix . 'soggetto_dichiarante']) ? sanitize_text_field($_POST[$prefix . 'soggetto_dichiarante']) : '';
...
function dci_admin_bar_incarichi_dip($wp_admin_bar) {
    ...
    $wp_admin_bar->add_node(array(
        'id'     => 'new_incarico_dip',
        'title'  => 'Incarichi conferiti e autorizzati ai dipendenti',
        'parent' => 'new-content',
        'href'   => admin_url('post-new.php?post_type=incarichi_dip'),
        'meta'   => array('class' => 'dci-admin-bar-inacarichi'),
    ));
}
