...
function recursionInsertTaxonomy($terms, $taxonomy, $parent = 0, &$order = 0) {
    ...
    foreach ($terms as $key => $value) {
        // Se $value è un array, $key è il nome del termine padre
        if (is_array($value)) {
            $term_name = $key;
        } else {
            // Altrimenti, $value è il termine
            $term_name = $value;
            $value = null; // niente figli
        }

        // Controllo esistenza termine
        $term_obj = term_exists($term_name, $taxonomy, $parent);

        if ($term_obj === 0 || $term_obj === null) {
            $term = wp_insert_term($term_name, $taxonomy, ['parent' => $parent]);
            if (is_wp_error($term)) {
                error_log("Errore inserimento termine '$term_name': " . $term->get_error_message());
                continue;
            }
        ...
        if (empty($term_id) || !is_numeric($term_id)) {
            error_log("Term_id non valido per termine '$term_name'");
            continue;
        }
        ...
        // Se ci sono figli ricorsivamente
        if (is_array($value)) {
